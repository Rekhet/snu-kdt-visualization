...
import { cancerMapping, heatmapMapping, divergingMapping } from '../data/chartDataMapping';
...
export default function OrganDetail() {
  ...
  if (!partData) {
    return <div className="p-10 text-center text-text-muted">Organ not found</div>;
  }
    ...
    <div className="w-full h-full flex flex-col bg-app-bg text-text-main transition-colors duration-200">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-panel-bg border-b border-panel-border">
        <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold border-l border-panel-border pl-4">{partData.label} Oncology Profile</h1>
        </div>
      </div>
        ...
        <div className="w-96 bg-panel-bg border-l border-panel-border overflow-y-auto z-10 shadow-xl">
            ...
            {cData ? (
                <div className="p-6 space-y-8 pb-12">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-input-bg p-3 rounded-lg">
                            <span className="block text-xs text-text-muted uppercase tracking-wide">Incidence</span>
                            <span className="font-bold text-brand">{cData.stats.incidence}</span>
                        </div>
                        <div className="bg-input-bg p-3 rounded-lg">
                            <span className="block text-xs text-text-muted uppercase tracking-wide">Survival (5yr)</span>
                            <span className="font-bold text-success">{cData.stats.survival}</span>
                        </div>
                    </div>
                    ...
                    {cancerMapping[organId] && (
                        <div>
                            <h3 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-3 flex items-center gap-2">
                                <svg className="w-4 h-4 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"></path></svg>
                    ...
                    <div>
                        ...
                        <ul className="space-y-2">
                            {cData.riskFactors.map(r => (
                                <li key={r} className="flex items-start gap-2 text-sm text-text-main">
                    ...
                    <div>
                        <h3 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-3 flex items-center gap-2">
                            <svg className="w-4 h-4 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                            Common Symptoms
                        </h3>
                        <ul className="space-y-2">
                            {cData.symptoms.map(s => (
                                <li key={s} className="flex items-start gap-2 text-sm text-text-main">
                                    <span className="mt-1.5 w-1 h-1 rounded-full bg-text-muted/50 flex-shrink-0"></span>
                                    {s}
                                </li>
                            ))}
                    ...
                    <div className="pb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {cData.treatment.map(t => (
                                <span key={t} className="text-xs bg-surface-hover text-text-main px-2 py-1 rounded border border-panel-border">
                                    {t}
                                </span>
                            ))}
                        </div>
                ...
                <div className="p-6 pb-12">
                    <h3 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-4">Common Pathologies</h3>
                    <ul className="space-y-2">
                        {partData.diseases.map(d => (
                            <li key={d} className="flex items-start gap-2 text-sm text-text-main">
