...
function Loader() {
  ...
  return (
    <Html
      center
      className="text-text-muted font-mono text-xs tracking-wider uppercase animate-pulse"
    >
      {progress.toFixed(0)}% Loading Model
    </Html>
  );
}
...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError)
      return (
        <Html center className="text-red-500 font-medium">
...
const ScrollScene = ({ interactionMode, modelGroupRef, setGridProgress }) => {
  ...
  useFrame(() => {
    ...
    const revealP = getPhaseProgress(
      progress,
      SCROLL_CONFIG.phases.EVENT_MODEL_REVEAL,
    );
    ...
    // 4. Grid Expansion
    const gridP = getPhaseProgress(
      progress,
      SCROLL_CONFIG.phases.EVENT_GRID_EXPAND,
    );
    ...
    if (modelGroupRef.current) {
      ...
      if (gridP > 0) {
        ...
        modelGroupRef.current.position.lerpVectors(
          heroStartPos,
          heroGridPos,
          gridEase,
        );
...
export default function Home({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const fadeProgress = getPhaseProgress(
        currentProgress,
        SCROLL_CONFIG.phases.EVENT_TITLE,
      );
      setTitleOpacity(1 - fadeProgress);

      // Chart Race Progress
      const chartP = getPhaseProgress(
        currentProgress,
        SCROLL_CONFIG.phases.EVENT_CHART_RACE,
      );
      setChartProgress(chartP);

      // Check if we are in the Single View Zone (Moved from ScrollScene)
      const inZone =
        currentProgress > SCROLL_CONFIG.phases.EVENT_MODEL_INTERACT.start &&
        currentProgress < SCROLL_CONFIG.phases.EVENT_MODEL_INTERACT.end;
  ...
  const isButtonHidden =
    progress < SCROLL_CONFIG.phases.EVENT_MODEL_REVEAL.start ||
    interactionMode ||
    isInScrollZone;
  ...
  return (
    <main className="relative w-full">
      ...
      <div
        ...
        className="relative bg-app-bg transition-colors duration-200"
      >
        ...
        <div
          className={`sticky top-16 h-[calc(100vh-4rem)] w-full overflow-hidden transition-all duration-300 z-10 ${showUI ? "pointer-events-auto" : "pointer-events-none"}`}
        >
          <Canvas
            shadows
            dpr={[1, 2]}
              ...
              <ErrorBoundary>
                <Suspense fallback={<Loader />}>
                  <HumanModel
                    theme={theme}
                    onPartSelect={(id) => {
                      if (showUI) handleSelectWrapper(id);
                    }}
                    onPartDoubleClick={(id) => {
                      if (showUI) handlePartDoubleClick(id);
                    }}
                    showWireframe={showWireframe}
                    onHoverChange={setHoveredPart}
                    prevalenceValue={
                      gridProgress > 0
                        ? (survivalTimeSeries.data[selectedCancer]?.values[
                            survivalTimeSeries.years.indexOf(selectedYear)
                          ] || 0) * 100
                        : null
                    }
                  />
        ...
        <div
          className={`fixed top-1/2 -translate-y-1/2 left-8 z-50 transition-opacity duration-500 pointer-events-auto flex flex-col gap-6 bg-panel-bg/90 backdrop-blur-md p-6 rounded-2xl border border-panel-border shadow-xl w-64 ${gridProgress > 0.8 ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        >
          <div>
          ...
          <div>
            ...
            <input
              ...
              max={
                survivalTimeSeries.years[survivalTimeSeries.years.length - 1]
              }
              step="1"
              value={selectedYear}
          ...
          <p className="text-[10px] text-text-muted italic leading-tight">
            Note: Data is scaled up by x100 to enhance visibility. 1 human mesh
            represents 0.01% of the actual population.
          </p>
        </div>
        ...
        <div
          style={{
            opacity:
              chartProgress > 0 && chartProgress < 0.9
                ? 1
        ...
        >
          <div className="pointer-events-auto">
            <BarChartRace
              ...
              visible={
                progress >= SCROLL_CONFIG.phases.EVENT_CHART_RACE.start &&
                progress <= SCROLL_CONFIG.phases.EVENT_CHART_RACE.end
              }
            />
          </div>
        </div>
