...
const NavigationControls = ({ cameraControls, onReset, isInteracting }) => {
  ...
  const btnClass =
    "bg-panel-bg-blur hover:bg-panel-bg text-text-main p-3 rounded-xl shadow-lg backdrop-blur-md transition-all active:scale-95 border border-panel-border flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 cursor-pointer pointer-events-auto";
  ...
  return (
    <div className="fixed bottom-10 left-8 z-[60] pointer-events-none">
      <button
        ...
        onClick={onReset}
        title={
          isInteracting ? "Reset / Exit Interaction" : "Enable Interaction"
        }
      >
        {isInteracting ? (
          <svg
