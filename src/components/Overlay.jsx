            ...
            {selectedPart && (
              ...
              >
                ...
                <div className="bg-brand/10 px-6 py-4 border-b border-brand/20 flex justify-between items-start">
                    <div>
                        <h2 className="text-2xl font-bold text-text-main">{selectedPart.label}</h2>
                        <span className="text-xs font-semibold text-brand bg-brand/20 px-2 py-0.5 rounded-full uppercase tracking-wider">{selectedPart.type}</span>
                    </div>
                    <button onClick={() => onSelect(null)} className="text-text-muted hover:text-text-main hover:bg-surface-hover rounded-full p-1 transition">
                ...
                <div className="p-6">
                    ...
                    {cancerMapping[selectedId] && (
                        <div className="mb-6">
                            <h3 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-3 flex items-center gap-2">
                                <svg className="w-3 h-3 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"></path></svg>
                                Incidence Trend (Age)
                            </h3>
                    ...
                    <div>
                        ...
                        {selectedPart.diseases.map(d => (
                            <span key={d} className="text-xs bg-surface-hover text-text-muted px-3 py-1.5 rounded-md border border-panel-border hover:border-brand hover:bg-brand/10 hover:text-brand transition cursor-default font-medium">
                            {d}
                            </span>
                        ))}
                        </div>
