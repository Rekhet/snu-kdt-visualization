...
const LineChart = ({
  ...
  useEffect(() => {
    const renderChart = (chartData) => {
      ...
      const allPoints = chartData.flatMap((s) =>
        s.values.map((v) => ({ ...v, series: s.label || "Series" })),
      );

      // Determine X Scale Type
      const isNumericX = allPoints.every(
      ...
      if (isNumericX) {
        ...
        x = d3.scalePoint().domain(xDomain).range([0, innerWidth]);
      }
      ...
      const validPoints = allPoints.filter(
        (d) => y(d.y) !== undefined && x(d.x) !== undefined,
      );
        ...
        if (index !== null) {
          ...
          highlightCircle
            ...
            .attr(
              "fill",
              d3.schemeCategory10[
                chartData.findIndex((s) => s.label === d.series) % 10
              ] || "steelblue",
            )
            .style("display", "block");
