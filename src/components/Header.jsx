  ...
  return (
    <header className="w-full h-16 bg-panel-bg border-b border-panel-border flex items-center justify-between px-6 z-50 relative shadow-sm transition-colors duration-200">
      ...
      <div
        ...
        onClick={() => navigate("/")}
      >
        <div className="w-8 h-8 bg-brand rounded-lg flex items-center justify-center text-text-inverse font-bold">
          S
        </div>
        <div>
          <h1 className="text-xl font-bold tracking-tight text-text-main leading-none">
        ...
        <div className="relative group w-64 md:w-80">
          ...
          {(isFocused || query) && query && (
            <div className="absolute top-full mt-2 left-0 w-full bg-panel-bg border border-panel-border rounded-xl overflow-hidden max-h-80 overflow-y-auto shadow-xl ring-1 ring-black/5 z-50">
              {filtered.length > 0 ? (
                <>
                  <div className="px-3 py-2 text-xs font-semibold text-text-muted uppercase tracking-wider bg-surface-hover">
                    Results
                  </div>
                  {filtered.map((part) => (
                    <button
                    ...
                    >
                      ...
                      <span className="text-xs text-text-muted group-hover:text-brand capitalize">
                        {part.type}
                      </span>
                    </button>
                  ))}
